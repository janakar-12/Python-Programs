...
while True:
    ...
    if new_pts.ravel()[0] > 400 or new_pts.ravel()[2] > 400:
        if new_pts.ravel()[0] > 550 or new_pts.ravel()[2] > 550:
            ...
            ytest_pos += 40
            cv2.putText(outp, 'gone at {}'.format(datetime.datetime.now().strftime('%H:%M')), (10, ytest_pos), cv2.FONT_HERSHEY_PLAIN, 3, (0, 255, 0))
        ...
        if new_pts.ravel()[0] < 50 or new_pts.ravel()[2] < 50:
            ...
            new_inp_img = backup_img.copy()
            ytest_pos += 40
            cv2.putText(outp, 'gone at {}'.format(datetime.datetime.now().strftime('%H:%M')), (10, ytest_pos), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255))
